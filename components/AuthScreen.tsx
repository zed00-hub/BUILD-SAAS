...
import { signInWithPopup, createUserWithEmailAndPassword, signInWithEmailAndPassword, sendEmailVerification, updateProfile } from 'firebase/auth';
            ...
            <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-100 p-8 relative z-10 animate-fade-in">
                ...
                <form onSubmit={async (e) => {
                    ...
                    if (isSignUp) {
                        ...
                        if (formData.countryCode === '+213') {
                            if (!/^[567]\d{8}$/.test(cleanPhone)) {
                                alert(t('invalid_phone') + " (Algérie: 9 digits starting with 5, 6, 7)");
                                setIsLoading(false);
                                return;
                            }
                        } else {
                    ...
                    try {
                        if (isSignUp) {
                            ...
                            // Save Name and Phone in Profile (Phone appended to DisplayName for now as a workaround)
                            // A better approach later is storing in Firestore 'users' collection.
                            const fullProfileName = `${formData.name} | ${formData.countryCode}${formData.phone}`;
                            await updateProfile(userCredential.user, {
                                displayName: fullProfileName
                            });
                        ...
                        <>
                            ...
                            <div className="input-group">
                                ...
                                <div className="flex gap-2">
                                    <select
                                        name="countryCode"
                                        className="px-3 py-2 rounded-xl border border-slate-200 bg-white focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition-all"
